    ...
    <v-container fluid class="create-game-container">
        ...
        <v-row>
            <v-col cols="12" class="d-flex justify-center">
                <h1 class="title">Set Map Range</h1>
            </v-col>
        </v-row>
        ...
        <v-row class="setting-row">
            <v-col cols="4">
                <v-btn @click="setDrawingMode('rectangle')" color="primary" outlined>Draw Rectangle</v-btn>
            </v-col>
            <v-col cols="4">
                <v-btn @click="setDrawingMode('circle')" color="primary" outlined>Draw Circle</v-btn>
            </v-col>
            <v-col cols="4">
                <v-btn @click="setDrawingMode('polygon')" color="primary" outlined>Draw Custom Shape</v-btn>
            </v-col>
        </v-row>
        ...
        <v-row>
            <v-col cols="12" class="d-flex justify-center">
                <v-btn @click="confirmSelection" color="success">Confirm Selection</v-btn>
            </v-col>
        </v-row>
    </v-container>
</template>
...
const confirmSelection = () => {
    ...
    if (selectedShape.value instanceof naver.maps.Rectangle) {
        ...
        scopeData = {
            type: "polygon",
            paths: selectedShape.value.getPaths().getArray().map((path) => path.toJSON()),
        };
    }
